...
class StoredValueDetail extends Component {
        ...
        Object.keys(listItems).map(key => {
            switch (key) {
                ...
                case 'afterTradeMoney':
                    _itemsMap.set(10, ['交易后余额', `￥${Number(listItems[key]).toFixed(2)}`]);
                    break;
                default:
                    break;
            }
    ...
    render() {
        ...
        return (
            <div className="animated fadeIn grey-back store-detail-container">
                <StorePointDetail type="store" itemsMap={itemsMap} history={history} number={this.getNumber(listItems)}/>
            </div>
        );
    }
}
